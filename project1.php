...
<p>The code for this project consists of several Python files, some of which you will need to read and understand in order to complete the assignment, and some of which you can ignore. You can download all the code and supporting files (including this description) as a zip archive.

</p><table border="0" cellpadding="10">
  <tbody><tr><td colspan="2"><b>Files you'll edit:</b></td></tr>

  <tr><td><code><a href="project_files/docs/search.html">search.py</a></code></td>
  <td>Where all of your search algorithms will reside.</td></tr>

  <tr><td><code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code></td>
  <td>Where all of your search-based agents will reside.</td></tr>


  <tr><td colspan="2"><b>Files you might want to look at:</b></td></tr>

  <tr><td><code><a href="project_files/docs/pacman.html">pacman.py</a></code></td>
  <td>The main file that runs Pacman games.  This file describes a Pacman GameState type, which you use in this project.</td></tr>

  <tr><td><code><a href="project_files/docs/game.html">game.py</a></code></td>
  ...
  <tr><td><code><a href="project_files/docs/util.html">util.py</a></code></td>
  ...
  <tr><td><code><a href="project_files/docs/graphicsDisplay.html">graphicsDisplay.py</a></code></td>
  <td>Graphics for Pacman</td></tr>

    <tr><td><code><a href="project_files/docs/graphicsUtils.html">graphicsUtils.py</a></code></td>
  <td>Support for Pacman graphics</td></tr>

  <tr><td><code><a href="project_files/docs/textDisplay.html">textDisplay.py</a></code></td>
  <td>ASCII graphics for Pacman</td></tr>

    <tr><td><code><a href="project_files/docs/ghostAgents.html">ghostAgents.py</a></code></td>
  <td>Agents to control ghosts</td></tr>

    <tr><td><code><a href="project_files/docs/keyboardAgents.html">keyboardAgents.py</a></code></td>
  <td>Keyboard interfaces to control Pacman</td></tr>

    <tr><td><code><a href="project_files/docs/layout.html">layout.py</a></code></td>
  <td>Code for reading layout files and storing their contents</td></tr>

</tbody></table>
<p>
</p><p><strong>What to submit:</strong> You will fill in portions of <code><a href="project_files/docs/search.html">search.py</a></code> and <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code> during the assignment.  You should submit these two files (only). <b>Do not zip these files. Do not leave debugging statements in. Do not submit any other files</b>. </p>
...
<p> The simplest agent in <a href="project_files/docs/searchAgents.html">searchAgents.py</a> is called the <code>GoWestAgent</code>, which always goes West (a trivial reflex agent).  This agent can occasionally win:
...
Note that <code><a href="project_files/docs/pacman.html">pacman.py</a></code> supports a number of options that can each be expressed in a long way (e.g., <code>--layout</code>) or a short way (e.g., <code>-l</code>).  You can see the list of all options and their default values via:

<pre>python pacman.py -h</pre>

Also, all of the commands that appear in this project also appear in <a href="project_files/commands.txt">commands.txt</a>, for easy copying and pasting.  In UNIX/Mac OS X, you can even run all these commands in order with <code>bash commands.txt</code>.
...
The command above tells the <code>SearchAgent</code> to use <code>tinyMazeSearch</code> as its search algorithm, which is implemented in <code><a href="project_files/docs/search.html">search.py</a></code>.  Pacman should navigate the maze successfully.
...
</p><p><em>Important note:</em> All of your search functions need to return a list of <em>actions</em> that will lead the agent from the start to the goal.  These actions all have to be legal moves (valid directions, no moving through walls).
...
</p><p><em>Hint:</em> Make sure to check out the <code>Stack, Queue</code> and <code>PriorityQueue</code> types provided to you in <code><a href="project_files/docs/util.html">util.py</a></code>!  </p>

<p><em><strong>Question 1 (2 points) </strong></em> Implement the depth-first search (DFS) algorithm in the
<code>depthFirstSearch</code> function in <code><a href="project_files/docs/search.html">search.py</a></code>. To make your algorithm <em>complete</em>, write the graph search version of DFS, which avoids expanding any already visited states (textbook section 3.5).
...
</p><p><em><strong>Question 2 (1 point) </strong></em> Implement the breadth-first search (BFS) algorithm in the
<code>breadthFirstSearch</code> function in <code><a href="project_files/docs/search.html">search.py</a></code>. Again, write a graph search algorithm that avoids expanding any already visited states. Test your code the same way you did for depth-first search.

</p><pre>python pacman.py -l mediumMaze -p SearchAgent -a fn=bfs</pre>
...
</p><p> <em>Note:</em> If you've written your search code generically, your code should work equally well for the eight-puzzle search problem (textbook section 3.2) without any changes.

</p><pre>python eightpuzzle.py</pre>

<h3> Varying the Cost Function </h3>

While BFS will find a fewest-actions path to the goal, we might want to find paths that are "best" in other senses.  Consider <code><a href="project_files/layouts/mediumDottedMaze.lay">mediumDottedMaze</a></code> and <code><a href="project_files/layouts/mediumScaryMaze.lay">mediumScaryMaze</a></code>.  By changing the cost function, we can encourage Pacman to find different paths.  For example, we can charge more for dangerous steps in ghost-ridden areas or less for steps in food-rich areas, and a rational Pacman agent should adjust its behavior in response.


<p><em><strong>Question 3 (2 points) </strong></em> Implement the uniform-cost graph search algorithm in
the  <code>uniformCostSearch</code> function in <code><a href="project_files/docs/search.html">search.py</a></code>. We encourage you to look through <code><a href="project_files/docs/util.html">util.py</a></code> for some data structures that may be useful in your implementation. You should now observe successful behavior in all three of the following layouts, where the agents below are all UCS agents that differ only in the cost function they use (the agents and cost functions are written for you):
...
<p> <em>Note:</em> You should get very low and very high path costs for the <code>StayEastSearchAgent</code> and <code>StayWestSearchAgent</code> respectively, due to their exponential cost functions (see <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code> for details).

</p><h3> A* search </h3>

<p><em><strong> Question 4 (3 points)  </strong></em> Implement A* graph search in the empty function <code>aStarSearch</code> in <code><a href="project_files/docs/search.html">search.py</a></code>. A* takes a heuristic function as an argument.  Heuristics  take two arguments: a state in the search problem (the main argument), and the problem itself (for reference information).  The <code>nullHeuristic</code> heuristic function in <code><a href="project_files/docs/search.html">search.py</a></code> is a trivial example.

</p><p>You can test your A* implementation on the original problem of finding a path through a maze to a fixed position using the Manhattan distance heuristic (implemented already as <code>manhattanHeuristic</code> in <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code>).

</p><pre>python pacman.py -l bigMaze -z .5 -p SearchAgent -a fn=astar,heuristic=manhattanHeuristic </pre>
...
</p><p>In <em>corner mazes</em>, there are four dots, one in each corner.  Our new search problem is to find the shortest path through the maze that touches all four corners (whether the maze actually has food there or not).  Note that for some mazes like <a href="project_files/layouts/tinyCorners.lay">tinyCorners</a>, the shortest path does not always go to the closest food first!  <em>Hint</em>: the shortest path through <code>tinyCorners</code> takes 28 steps.

</p><p><em><strong> Question 5 (2 points)  </strong></em> Implement the <code>CornersProblem</code> search problem in <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code>.  You will need to choose a state representation that encodes all the information necessary to detect whether all four corners have been reached. Now, your search agent should solve:

</p><pre>python pacman.py -l tinyCorners -p SearchAgent -a fn=bfs,prob=CornersProblem</pre>
...
</p><p>Our implementation of <code>breadthFirstSearch</code> expands just under 2000 search nodes on <a href="project_files/layouts/mediumCorners.lay">mediumCorners</a>.  However, heuristics (used with A* search) can reduce the amount of searching required.

</p><p><em><strong> Question 6 (3 points)  </strong></em> Implement a heuristic for the <code>CornersProblem</code> in <code>cornersHeuristic</code>.  Grading: inadmissible heuristics will get <em>no</em> credit.   1 point for any admissible heuristic.  1 point for expanding fewer than 1600 nodes.  1 point for expanding fewer than 1200 nodes.   Expand fewer than 800, and you're doing great!

</p><pre>python pacman.py -l mediumCorners -p AStarCornersAgent -z 0.5</pre>
...
</p><p> <em>Note:</em> <code>AStarCornersAgent</code> is a shortcut for <code>-p SearchAgent -a fn=aStarSearch,prob=CornersProblem,heuristic=cornersHeuristic</code>.

</p><h3> Eating All The Dots </h3>

Now we'll solve a hard search problem: eating all the Pacman food in as few steps as possible.  For this, we'll need a new search problem definition which formalizes the food-clearing problem: <code>FoodSearchProblem</code> in <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code> (implemented for you).  A solution is defined to be a path that collects all of the food in the Pacman world.  For the present project, solutions do not take into account any ghosts or power pellets; solutions only depend on the placement of walls, regular food and Pacman.  (Of course ghosts can ruin the execution of a solution!  We'll get to that in the next project.)  If you have written your general search methods correctly, <code>A*</code> with a null heuristic (equivalent to uniform-cost search) should quickly find an optimal solution to <a href="project_files/layouts/testSearch.lay">testSearch</a> with no code change on your part (total cost of 7).
...
</p><p>You should find that UCS starts to slow down even for the seemingly simple <code><a href="project_files/layouts/tinySearch.lay">tinySearch</a></code>.  As a reference, our implementation takes 2.5 seconds to find a path of length 27 after expanding 4902 search nodes.

</p><p> <em><strong> Question 7 (5 points)  </strong></em> Fill in <code>foodHeuristic</code> in <code><a href="project_files/docs/searchAgents.html">searchAgents.py</a></code> with a <emph> consistent </emph> heuristic for the <code>FoodSearchProblem</code>.  Try your agent on the <code>trickySearch</code> board:
...
<table align="center" border="1" cellspacing="1" cellpadding="5">
    <tbody><tr><th>Fewer nodes than:</th><th>Points</th></tr>
...
</p><h3> <a name="Glossary">Object Glossary</a> </h3>
...
<p> Here's a glossary of the key objects in the code base related to search problems, for your reference:
      ...
      <dd>A SearchProblem is an abstract object that represents the state space, successor function, costs, and goal state of a problem.  You will interact with any SearchProblem only through the methods defined at the top of <code><a href="project_files/docs/search.html">search.py</a></code></dd>
